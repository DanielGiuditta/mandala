...
const version = process.env.MANDALA_GATEWAY_VERSION?.trim()
const expectedProjectRef = "nzlajptokbcgeaifgnoq"
const destination = requestedDestination ||
  (version ? `gateway/MandalaGatewaySetup-${version}.exe` : "gateway/MandalaGatewaySetup.exe")
...
if (!version || !/^\d+\.\d+\.\d+$/.test(version)) {
  throw new Error("MANDALA_GATEWAY_VERSION must use major.minor.patch format.")
}
...
const expectedFilename = `MandalaGatewaySetup-${version}.exe`
...
const manifestResponse = await fetch(
  `${supabaseUrl}/storage/v1/object/desktop-agent-releases/gateway/release.json`,
  {
    method: "POST",
    headers: {
      apikey: serviceRoleKey,
...
console.log("Published verified release manifest to desktop-agent-releases/gateway/release.json")
